...
addParameter(p, 'alg', 'scca', @(x) any(validatestring(x,{'scca', 'scca_lasso'})));
...
[data_fmri, ROI, AAL_ROI_VI] = load_and_apply_mask(params.type_fmri, params.type_mask, data_fmri);
...
for i=1:params.num_comps
    [best_lambda_u, best_lambda_v] = tune_scca(params.tuning_params, params.uu, params.vv, params.alg, data_fmri_res, data_snp_res, labels, num_SZ);
    [u, v] = run_scca(best_lambda_u, best_lambda_v, params.alg, data_fmri_res, data_snp_res);
    U(:,i) = u;V(:,i) = v;
    ...
    test_group_diff(i, coeff_fmri_SZ, coeff_fmri_NC, coeff_snp_SZ, coeff_snp_NC);
...
for i=1:params.num_comps
    ...
    result(i)=abs(rho);
    fprintf('the %d-th resulting correlation is %f(p=%f), at lambda_u=%f(%d non-0s),lambda_v=%f(%d non-0s)\n.',i,result(i),pval,best_lambda_U(i), sum(U(:,i)~=0),best_lambda_V(i),sum(V(:,i)~=0));
end

% evaluation and output
test_group_diff(params.num_comps, coeff_fmri_SZ, coeff_fmri_NC, coeff_snp_SZ, coeff_snp_NC);
%plot_loadings(params.num_comps, coeff_fmri_SZ, coeff_fmri_NC, coeff_snp_SZ, coeff_snp_NC, U, V);
write_components(params.type_fmri, params.type_mask, params.num_comps, U, V, best_lambda_U, best_lambda_V, ROI);
%}
